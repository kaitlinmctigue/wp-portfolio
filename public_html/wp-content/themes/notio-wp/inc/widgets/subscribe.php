...
class thb_subscribe_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __('A widget that gathers email addresses.','bronx')
		);
	
		parent::__construct(
			'thb_subscribe_widget',
			__( 'Fuel Themes - Subscribe Widget' , 'bronx' ),
			$widget_ops
		);
		...
		?>
			...
			jQuery(document).ready(function($) {
                ...
                <form id="newsletter-form" action="#" method="post" class="row" data-target="<?php echo THB_THEME_ROOT; ?>/inc/subscribe_save.php'; ?>">   
                	<div class="small-9 columns"><input placeholder="<?php _e("YOUR E-MAIL",'bronx'); ?>" type="text" name="widget_subscribe" id="widget_subscribe"></div>
									<div class="small-3 columns"><button type="submit" name="submit" class="btn white"><?php _e("GO",'bronx'); ?></button></div>
                </form>
	...
	function form( $instance ) {
		...
		<p>
			<?php _e('You can find the collected emails <a href="'.THB_THEME_ROOT.'/inc/subscribers.csv" target="_blank">here</a>', 'bronx'); ?>
		</p>
	<?php
	}
}
