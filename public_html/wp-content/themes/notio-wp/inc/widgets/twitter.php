...
class widget_thbtwitter extends WP_Widget { 
	function __construct() {
		$widget_ops = array(
			...
			'description' => __('Display your Tweets','bronx')
		);
	
		parent::__construct(
			'thb_twitter_widget',
			__( 'Fuel Themes - Twitter Widget' , 'bronx' ),
			$widget_ops
		);
	...
	function widget($args, $instance) {
		...
		<a href="http://twitter.com/<?php echo ot_get_option('twitter_bar_username'); ?>" class="btn small twitter" target="_blank"><i class="icon-budicon-841"></i> <?php _e('Follow <strong>@'.esc_attr($username).'</strong>', 'bronx'); ?> <i class="fa fa-twitter"></i></a>
		<?php
		echo $after_widget;
	}
