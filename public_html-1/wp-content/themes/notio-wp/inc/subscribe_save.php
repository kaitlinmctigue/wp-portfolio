...
if (is_email($email)) 
{
	...
	else
	{
		...
		if(in_array($email, $stack))
		{
			...
			_e('<strong>Oh snap!</strong> That email address is already subscribed!', 'bronx');
			echo "</div>";
		}
		else
		{
			...
			_e("<strong>Well done!</strong> Your address has been added", 'bronx');
			echo "</div>";
		}
	}
}
else
{
	...
	_e("<strong>Oh snap!</strong> Please enter a valid email address", 'bronx');
	echo "</div>";
}
