...
<tr class="shipping">
	<th colspan="3"><?php
		if ( $show_package_details ) {
			printf( __( 'Shipping #%d','bronx' ), $index + 1 );
		} else {
			_e( 'Shipping and Handling','bronx' );
		}
	?></th>
	<td>
		...
		<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
				...
				<p><?php _e( 'Please use the shipping calculator to see available shipping methods.', 'bronx' ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p><?php _e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', 'bronx' ); ?></p>

			<?php else : ?>

				<p><?php _e( 'Please fill in your details to see available shipping methods.', 'bronx' ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<div class="woocommerce-info"><p>' . __( 'There don&lsquo;t seem to be available shipping methods. Please double check your address, or contact us if you need any help.', 'bronx' ) . '</p></div>'
				); ?>

			<?php else : ?>

				<?php echo apply_filters( 'woocommerce_no_shipping_available_html',
					'<p>' . __( 'There don&lsquo;t seem to be available shipping methods. Please double check your address, or contact us if you need any help.', 'bronx' ) . '</p>'
				); ?>
		...
		<?php if ( $show_package_details ) : ?>
			<?php
				...
				echo '<p class="woocommerce-shipping-contents"><small>' . __( 'Shipping', 'bronx' ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
			?>
		<?php endif; ?>
	</td>
</tr>